"""Create daybreak.db - the running database for learn-mcp-with-phoebe.

Same schema and data as the Daybreak database in learn-sql-with-phoebe,
so everything you learned there (and the March dip) carries over.

Usage:
    python seed_daybreak.py          # writes ./daybreak.db, replacing any old copy
"""
...
def main() -> None:
    ...
    con.close()
    print(f"daybreak.db created at {DB_PATH} ({n_orders} orders)")
